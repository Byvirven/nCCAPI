...
public:
    Impl() {
        ...
        session = std::make_unique<KrakenFuturesSession>(options, configs);
    }

    std::vector<Instrument> get_instruments() {
        std::vector<Instrument> instruments;
        ccapi::Request request(ccapi::Request::Operation::GET_INSTRUMENTS, "kraken-futures");
                ...
                if (event.getType() == ccapi::Event::Type::RESPONSE) {
                    for (const auto& message : event.getMessageList()) {
                        if (message.getType() == ccapi::Message::Type::GET_INSTRUMENTS) {
                            ...
                            // std::cerr << "KrakenFutures Error: " << message.getElementList()[0].getValue(CCAPI_ERROR_MESSAGE) << std::endl;
                            return instruments;
                        }
                    }
                }
...
private:
    std::unique_ptr<KrakenFuturesSession> session;
};

KrakenFutures::KrakenFutures() : pimpl(std::make_unique<Impl>()) {}
