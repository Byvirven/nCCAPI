...
public:
    ...
    std::vector<Instrument> get_instruments() {
        ...
        ccapi::Request request(ccapi::Request::Operation::GET_INSTRUMENTS, "binance-us");
                ...
                if (event.getType() == ccapi::Event::Type::RESPONSE) {
                    for (const auto& message : event.getMessageList()) {
                        if (message.getType() == ccapi::Message::Type::GET_INSTRUMENTS) {
                            ...
                            // std::cerr << "BinanceUs Error: " << message.getElementList()[0].getValue(CCAPI_ERROR_MESSAGE) << std::endl;
                            return instruments;
                        }
                    }
                }
