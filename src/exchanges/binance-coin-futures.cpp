...
public:
    Impl() {
        ...
        session = std::make_unique<BinanceCoinFuturesSession>(options, configs);
    }
                    ...
                    for (const auto& message : event.getMessageList()) {
                        if (message.getType() == ccapi::Message::Type::GET_INSTRUMENTS) {
                            for (const auto& element : message.getElementList()) {
                                ...
                                // Check contractType or similar if needed for perpetual (swap) vs future
...
private:
    std::unique_ptr<BinanceCoinFuturesSession> session;
};

BinanceCoinFutures::BinanceCoinFutures() : pimpl(std::make_unique<Impl>()) {}
