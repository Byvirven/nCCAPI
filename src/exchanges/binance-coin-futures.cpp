...
class BinanceCoinFutures::Impl {
public:
    Impl(std::shared_ptr<UnifiedSession> s) : session(s) {}

    std::vector<Instrument> get_instruments() {
        std::vector<Instrument> instruments;
        ccapi::Request request(ccapi::Request::Operation::GET_INSTRUMENTS, "binance-coin-futures");
                    ...
                    for (const auto& message : event.getMessageList()) {
                        if (message.getType() == ccapi::Message::Type::GET_INSTRUMENTS) {
                            for (const auto& element : message.getElementList()) {
                                ...
                                instrument.type = "future"; // COIN-M Futures
    ...
    std::vector<Candle> get_historical_candles(const std::string& instrument_name,
        ...
        while (true) {
            // Binance Coin Futures -> /dapi/v1/klines
            ccapi::Request request(ccapi::Request::Operation::GENERIC_PUBLIC_REQUEST, "binance-coin-futures", "", "");
            ...
            request.appendParam({
                ...
                {CCAPI_HTTP_PATH, "/dapi/v1/klines"},
                    ...
                    if (event.getType() == ccapi::Event::Type::RESPONSE) {
                        for (const auto& message : event.getMessageList()) {
                            if (message.getType() == ccapi::Message::Type::GENERIC_PUBLIC_REQUEST) {
                                ...
                                // std::cout << "[DEBUG] Binance Coin Futures Error: " << message.toString() << std::endl;
                                success = true;
                            }
                        }
                    }
...
BinanceCoinFutures::BinanceCoinFutures(std::shared_ptr<UnifiedSession> session) : pimpl(std::make_unique<Impl>(session)) {}
BinanceCoinFutures::~BinanceCoinFutures() = default;
