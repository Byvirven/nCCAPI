...
public:
    ...
    std::vector<Candle> get_historical_candles(const std::string& instrument_name,
        ...
        while (true) {
            // Binance USDS Futures -> /fapi/v1/klines
            ccapi::Request request(ccapi::Request::Operation::GENERIC_PUBLIC_REQUEST, "binance-usds-futures", "", "");
            ...
            request.appendParam({
                ...
                {CCAPI_HTTP_PATH, "/fapi/v1/klines"},
                    ...
                    if (event.getType() == ccapi::Event::Type::RESPONSE) {
                        for (const auto& message : event.getMessageList()) {
                            if (message.getType() == ccapi::Message::Type::GENERIC_PUBLIC_REQUEST) {
                                ...
                                // std::cout << "[DEBUG] Binance USDS Futures Error: " << message.toString() << std::endl;
                                success = true;
                            }
                        }
                    }
...
BinanceUsdsFutures::BinanceUsdsFutures(std::shared_ptr<UnifiedSession> session) : pimpl(std::make_unique<Impl>(session)) {}
...
std::vector<Candle> BinanceUsdsFutures::get_historical_candles(const std::string& instrument_name,
                                                     const std::string& timeframe,
                                                     int64_t from_date,
                                                     int64_t to_date) {
