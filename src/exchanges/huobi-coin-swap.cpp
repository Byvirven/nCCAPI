...
public:
    Impl() {
        ...
        session = std::make_unique<HuobiCoinSwapSession>(options, configs);
    }
...
private:
    std::unique_ptr<HuobiCoinSwapSession> session;
};

HuobiCoinSwap::HuobiCoinSwap() : pimpl(std::make_unique<Impl>()) {}
