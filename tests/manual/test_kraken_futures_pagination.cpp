...
int main() {
    ...
    std::string exchange_name = "kraken-futures";
    // Kraken Futures limit is ~1200-5000. Request 6000 minutes.
    int duration_minutes = 6000;
    ...
    try {
        ...
        if (!candles.empty()) {
             ...
             if (candles.size() < 5001) {
                 std::cout << RED << "[FAIL] Received " << candles.size() << " candles. Pagination likely not working (Limit < 6000)." << RESET << std::endl;
             } else {
                 std::cout << GREEN << "[PASS] Received " << candles.size() << " candles." << RESET << std::endl;
             }
        } else {
