...
int main() {
    ...
    KucoinFutures exchange(session);
    std::string symbol = "XBTUSDTM"; // Check symbol
    int64_t now = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
    // 2000 mins > 200 limit (or whatever default)
    int64_t from = now - (2000 * 60 * 1000);
    int64_t to = now;

    std::cout << "Testing exchange: kucoin-futures" << std::endl;
    ...
    if (candles.size() > 205) {
        std::cout << "\033[32m[PASS] Received " << candles.size() << " candles (Expected > 200).\033[0m" << std::endl;
    } else {
        std::cout << "\033[31m[FAIL] Received " << candles.size() << " candles (Expected > 200).\033[0m" << std::endl;
    }
