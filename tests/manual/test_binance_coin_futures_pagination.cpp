...
int main() {
    ...
    BinanceCoinFutures exchange(session);
    std::string symbol = "BTCUSD_PERP"; // Coin-M Perp
    ...
    std::cout << "Testing exchange: binance-coin-futures" << std::endl;
    ...
    if (candles.size() > 1005) {
        ...
        std::cout << "\033[31m[FAIL] Received " << candles.size() << " candles (Expected > 1000).\033[0m" << std::endl;
    }
