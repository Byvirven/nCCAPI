...
int main() {
    ...
    std::string exchange_name = "bitstamp";
    // Bitstamp limit is 1000. We request 2500 minutes.
    int duration_minutes = 2500;
    std::string symbol = "btcusd"; // Lowercase for Bitstamp v2 OHLC
    ...
    try {
        ...
        if (!candles.empty()) {
             ...
             if (candles.size() < 1001) {
                 std::cout << RED << "[FAIL] Received " << candles.size() << " candles. Pagination likely not working (Limit is 1000)." << RESET << std::endl;
             } else {
                 std::cout << GREEN << "[PASS] Received " << candles.size() << " candles (Expected > 1000)." << RESET << std::endl;
             }
        } else {
