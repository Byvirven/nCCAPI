...
int main() {
    ...
    MexcFutures exchange(session);
    std::string symbol = "BTC_USDT"; // Check symbol format
    int64_t now = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
    // 2500 mins > 2000 limit
    int64_t from = now - (2500 * 60 * 1000);
    int64_t to = now;

    std::cout << "Testing exchange: mexc-futures" << std::endl;
    ...
    if (candles.size() > 2005) {
        std::cout << "\033[32m[PASS] Received " << candles.size() << " candles (Expected > 2000).\033[0m" << std::endl;
    } else {
        std::cout << "\033[31m[FAIL] Received " << candles.size() << " candles (Expected > 2000).\033[0m" << std::endl;
    }
