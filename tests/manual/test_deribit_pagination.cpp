...
int main() {
    ...
    std::string exchange_name = "deribit";
    // Deribit limit is 4000? We request 5000 minutes.
    int duration_minutes = 5000;
    ...
    try {
        ...
        if (!candles.empty()) {
             ...
             if (candles.size() < 4001) {
                 std::cout << RED << "[FAIL] Received " << candles.size() << " candles. Pagination likely not working (Limit might be ~4000)." << RESET << std::endl;
             } else {
                 std::cout << GREEN << "[PASS] Received " << candles.size() << " candles (Expected > 4000)." << RESET << std::endl;
             }
        } else {
