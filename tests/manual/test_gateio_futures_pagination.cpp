...
int main() {
    ...
    std::string exchange_name = "gateio-perpetual-futures";
    // Gateio limit is 2000. We request 4000 minutes.
    int duration_minutes = 4000;
    ...
    try {
        ...
        if (!candles.empty()) {
             ...
             if (candles.size() < 2001) {
                 std::cout << RED << "[FAIL] Received " << candles.size() << " candles. Pagination likely not working (Limit is 2000)." << RESET << std::endl;
             } else {
                 std::cout << GREEN << "[PASS] Received " << candles.size() << " candles (Expected > 2000)." << RESET << std::endl;
             }
        } else {
