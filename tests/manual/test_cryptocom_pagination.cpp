...
int main() {
    ...
    std::string exchange_name = "cryptocom";
    // Crypto.com limit is likely 300. We request 800 minutes.
    int duration_minutes = 800;
    ...
    try {
        ...
        if (!candles.empty()) {
             ...
             if (candles.size() < 301) {
                 std::cout << RED << "[FAIL] Received " << candles.size() << " candles. Pagination likely not working (Limit is ~300)." << RESET << std::endl;
             } else {
                 std::cout << GREEN << "[PASS] Received " << candles.size() << " candles (Expected > 300)." << RESET << std::endl;
             }
        } else {
