...
int main() {
    ...
    std::string exchange_name = "kraken";
    // Kraken limit is 720. We request 2000 minutes.
    int duration_minutes = 2000;
    std::string symbol = "XXBTZUSD"; // Use Kraken internal symbol
    ...
    try {
        ...
        if (!candles.empty()) {
             ...
             if (candles.size() < 721) {
                 std::cout << RED << "[FAIL] Received " << candles.size() << " candles. Pagination likely not working (Limit is ~720)." << RESET << std::endl;
             } else {
                 std::cout << GREEN << "[PASS] Received " << candles.size() << " candles (Expected > 720)." << RESET << std::endl;
             }
        } else {
